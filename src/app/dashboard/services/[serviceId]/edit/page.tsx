...
import { getServiceById, updateService } from '@/lib/services-api';
import { getStaff } from '@/lib/staff-api';
import { canEditService, getServiceCategoryOptions } from '@/lib/service-utils';
import { isMockModeEnabled } from '@/lib/dashboard-auth';
import type { SalonService, ServiceCategory, ServiceStatus } from '@/types/services';

export default function EditServicePage() {
  const user = useDashboardUser();
  if (!canEditService(user.role)) return <PermissionDenied />;
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await getServiceById(params.serviceId);
        if (data) {
          setService(data);
          setForm({
            name: data.name,
            ...
            assignedStaff: data.assignedStaffNames ?? [],
          });
        }
      } catch (e) {
  ...
  if (loading) return <DashboardLoading message="Loading service..." />;
  if (error) return <DashboardError message={error} />;
  if (!service) return <DashboardError message="Service not found." />;
