...
import { createPackage, getMockServices, getServices } from '@/lib/services-api';
import { canCreatePackage } from '@/lib/service-utils';
import { isMockModeEnabled } from '@/lib/dashboard-auth';
import type { PackageStatus } from '@/types/services';

export default function NewPackagePage() {
  const user = useDashboardUser();
  if (!canCreatePackage(user.role)) return <PermissionDenied />;
    ...
    try {
      ...
      setResult({
        ...
        text: isMockModeEnabled()
          ? 'Package created successfully (demo mode).'
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Add Package</h1>
          <p className="text-sm text-gray-500">Create a new bundled beauty package.</p>
        </div>
      </div>
